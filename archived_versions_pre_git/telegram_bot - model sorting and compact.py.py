        ...
        try:
            ...
            for html_file in html_files:
                try:
                    ...
                    # Extract model from content (look for "Model: " pattern)
                    if 'Model:' in content:
                        lines = content.split('\n')
                        for line in lines:
                            if 'Model:' in line:
                                # Extract model name from HTML
                                model_start = line.find('Model:') + 6
                                model_end = line.find('</strong>', model_start)
                                if model_end == -1:
                                    model_end = line.find('</', model_start)
                                if model_end == -1:
                                    model_end = len(line)
                                model = line[model_start:model_end].strip()
                                model = model.replace('<strong>', '').replace('</strong>', '').strip()
                                if model:
        ...
        <div class="content">
            ...
            if html_files:
                ...
                for file_path in html_files:
                    ...
                    # Try to extract title, URL, and model from HTML file content
                    used_model = "Unknown"
                    try:
                    ...
                    index_html += f"""
                <div class="report-card" data-title="{display_name}" data-time="{mtime.timestamp()}" data-size="{file_size}" data-filename="{file_path.name}" data-model="{used_model}">
