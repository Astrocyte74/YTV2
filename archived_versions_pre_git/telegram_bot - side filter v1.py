...
<body>
    <div class="container">
        ...
        <div class="controls-bar">
                <div class="controls-row">
                    ...
                    <div class="controls-group">
                        ...
                        <div class="filter-info" style="color: #6c757d; font-size: 0.9em; margin-left: 12px;">
                        ...
                        <div class="export-dropdown">
                            ...
                            <div class="export-dropdown-content">
                                <button onclick="exportReports('pdf')">📄 PDF</button>
                                <button onclick="exportReports('md')">📝 Markdown</button>
                                <button onclick="exportReports('json')">🔧 JSON</button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="content">
                ...
                <div class="sidebar-filter">
                    ...
                    <div class="sidebar-content" id="sidebarContent">
                        ...
                        <div class="filter-section">
                            <div class="filter-section-title">AI Models</div>
                            <div id="modelFilters">"""
            ...
            for model, count in sorted(model_usage.items()):
                ...
                index_html += f"""
                                <div class="filter-item">
                                    <input type="checkbox" class="filter-checkbox" id="model_{model_id}" 
                                           value="{model}" onchange="applyFilters()" checked>
                                    <label for="model_{model_id}" class="filter-label">{display_name}</label>
            ...
            index_html += """
                        ...
                        <!-- Channels Section -->
                        <div class="filter-section">
                            <div class="filter-section-title">Channels</div>
                            <div id="channelFilters">"""
                            
            # Add channel filter checkboxes
            for channel, count in sorted(channel_usage.items()):
                channel_id = channel.replace(' ', '_').replace('-', '_').replace('&', '_').replace('.', '_')
                # Truncate long channel names for display
                display_channel = channel if len(channel) <= 30 else channel[:27] + "..."
                index_html += f"""
                                <div class="filter-item">
                                    <input type="checkbox" class="filter-checkbox" id="channel_{channel_id}" 
                                           value="{channel}" onchange="applyFilters()" checked>
                                    <label for="channel_{channel_id}" class="filter-label" title="{channel}">{display_channel}</label>
                                    <span class="filter-count">({count})</span>
                                </div>"""
            ...
            if html_files:
                    ...
                    try:
                        ...
                        for pattern in channel_patterns:
                            ...
                            if channel_match:
                                ...
                                logger.info(f"🔍 Extracted channel '{used_channel}' from {file_path.name}")
                                break
                ...
                <div class="report-card" data-title="{display_name}" data-time="{mtime.timestamp()}" data-filename="{file_path.name}" data-model="{used_model}" data-channel="{used_channel}" data-summary-type="{summary_type}">
                    <div class="card-thumbnail">
                        ...
                        <button class="card-delete-btn" onclick="deleteSingleCard('{file_path.name}')" title="Delete this report" style="position: absolute; top: 12px; right: 12px; z-index: 10;">×</button>
                        {thumbnail_html}
                    </div>
    ...
    <script>
        ...
        function applyFilters() {
            ...
            document.querySelectorAll('#modelFilters input[type="checkbox"]:checked').forEach(function(checkbox) {
                selectedModels.push(checkbox.value);
            });
            
            // Get selected channels
            const selectedChannels = [];
            document.querySelectorAll('#channelFilters input[type="checkbox"]:checked').forEach(function(checkbox) {
            ...
            cards.forEach(function(card) {
                ...
                const modelMatch = selectedModels.length === 0 || selectedModels.includes(cardModel);
                const channelMatch = selectedChannels.length === 0 || selectedChannels.includes(cardChannel);
            ...
            console.log(`Filtered: ${visibleCount} cards visible`);
        }
        
        function clearAllFilters() {
            // Check all model checkboxes
            document.querySelectorAll('#modelFilters input[type="checkbox"]').forEach(function(checkbox) {
                checkbox.checked = true;
            });
            
            // Check all channel checkboxes
            document.querySelectorAll('#channelFilters input[type="checkbox"]').forEach(function(checkbox) {
                checkbox.checked = true;
            });
        ...
        // Replace old filterByChip function to work with sidebar
        function filterByChip(chipElement, filterValue) {
            // This function is now deprecated but kept for compatibility
            // New filtering is handled by applyFilters()
            console.log('Chip filtering deprecated, using sidebar filters');
        }
    </script>
</body>
</html>"""
