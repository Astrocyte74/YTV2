...
class YouTubeSummarizer:
    ...
    async def _condense_for_tts(self, text: str) -> str:
        ...
        # Calculate target length - aim for 90% of original to minimize cuts
        target_chars = min(4000, int(len(text) * 0.9))
        target_words = target_chars // 5  # Rough chars-to-words conversion
        
        condense_prompt = f"""
        Carefully trim this {len(text)}-character summary to approximately {target_words} words ({target_chars} characters) while preserving ALL essential content:

        CRITICAL - Preserve completely:
        - All main findings and conclusions
        - Every specific number, price, measurement, and statistic
        - The complete final recommendation with reasoning
        - Key comparisons and trade-offs
        - All major topic sections

        MINIMAL trimming only:
        - Remove redundant phrasing ("as mentioned", "it's worth noting")
        - Combine similar points more concisely
        - Shorten transition phrases while keeping natural flow
        - Use slightly more direct language

        Do NOT eliminate entire sections or major points. This should read like a tighter version of the original, not a different summary.
    ...
    async def generate_tts_audio(self, text: str, output_filename: str = None) -> Optional[str]:
        ...
        try:
            ...
            # Step 1: Handle TTS character limit with "summarize the summary" approach
            tts_text = text
            if len(text) > 4090:  # Only condense when actually exceeding OpenAI's 4096 limit
                print(f"📝 Audio summary is {len(text)} characters (exceeds OpenAI's 4096 TTS limit)")
                print("🔄 Creating condensed version for TTS while preserving key content...")
                tts_text = await self._condense_for_tts(text)
                print(f"✅ Condensed summary: {len(tts_text)} chars (preserves main findings & conclusions)")
            else:
