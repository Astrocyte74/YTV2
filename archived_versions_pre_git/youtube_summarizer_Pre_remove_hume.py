...
class YouTubeSummarizer:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if not transcript_text:
                    ...
                    if description and len(description.strip()) > 50:
                        ...
                        print(f"⚠️  No captions/transcript found, using video description as fallback ({len(description)} chars)")
                    else:
                    ...
                    if len(description) > len(transcript_text):
                        transcript_text = description
                        print(f"⚠️  Transcript too short ({len(transcript_text)} chars), using video description instead ({len(description)} chars)")
                    else:
    ...
    async def generate_summary(self, transcript: str, metadata: Dict, 
        ...
        prompts = {
            ...
            **Writing Guidelines:**
            ...
            Create an audio-optimized summary designed for text-to-speech playback. Write as a single flowing narrative without section headers or labels.

            **Structure (as continuous speech, 180-250 words):**
            
            Flow the content naturally using these elements:
            - Open with "This video covers..." or "In this video, [speaker] explains..." 
            - Present core information using natural transitions ("First," "Next," "Additionally," "However")
            - Include specific details, numbers, or steps mentioned
            - Weave in practical significance with phrases like "What this means for you is..." or "The practical takeaway here is..."
            - Conclude with "The key point to remember is..." followed by the main insight

            **Write as one continuous paragraph** - no section breaks, headers, or labels that would sound awkward when spoken aloud.
    ...
    def generate_tts_audio(self, text: str, output_filename: str = None) -> Optional[str]:
        """Generate TTS audio using Hume AI API
