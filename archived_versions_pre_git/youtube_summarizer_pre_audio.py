    ...
    async def generate_summary(self, transcript: str, metadata: Dict, 
        ...
        prompts = {
            ...
            **Writing Guidelines:**
            ...
            Write a 120–180 word MARKDOWN brief as a single paragraph. Do not use headings, lists, emojis, or line breaks.

Structure (merge seamlessly into one paragraph):
- Start with one crisp thesis sentence stating what the video does and why it matters.
- Follow with 3–5 short sentences giving concrete specifics (models, versions, figures, steps). Include [mm:ss] timestamps only when they can be taken directly from the transcript; otherwise omit them.
- Add 1–2 sentences on the practical significance (how a developer/team would use this).
- Add one sentence of explicit caveat or limitation mentioned in the transcript (write **Unknown** if none).
- End with a final sentence beginning **Bottom line:** that states the single key takeaway.

Style rules:
- Neutral, precise tone; British/Canadian spelling.
- No speculation or external knowledge; if the transcript doesn’t support a claim, write **Unknown**.
- Use proper nouns as given; keep sentences short and concrete.
- Avoid mentioning a target audience or giving generic advice.
""",
