...
class YouTubeSummarizer:
    ...
    async def generate_summary(self, transcript: str, metadata: Dict, 
        ...
        Args:
            ...
            summary_type: Type of summary ('comprehensive', 'brief', 'bullet-points', 'key-insights')
        ...
        prompts = {
            ...
            You are formatting a precise, scannable MARKDOWN summary for a YouTube video. Use ONLY the sections below, in this exact order and with these emojis. Do not add extra sections or prose.

            # 📝 DETAILED SUMMARY
            (one sentence that captures what the video does and why it matters; no emojis here)

            ## 📋 OVERVIEW
            2–3 sentences. Say what the video covers and why it’s useful. Neutral tone. No hype.

            ## ▶️ CONTEXT SNAPSHOT
            - **Title:** {metadata.get('title', 'Unknown')}
            - **Channel:** {metadata.get('uploader', 'Unknown')}
            - **Upload Date:** {metadata.get('upload_date', 'Unknown')}
            - **Duration:** {{convert seconds in the context above to mm:ss}}
            - **Link:** {metadata.get('url', 'Unknown')}

            ## 🔑 KEY POINTS
            3–7 bullets with the most important specifics. If you can infer timestamps, prefix items like `[05:12]`; otherwise omit. Each ≤ 20 words.

            ## 🕒 TIMELINE / CHAPTERS (optional)
            3–6 bullets in chronological order with approximate timestamps if inferable. Omit if the video is not sequential/tutorial‑style.

            ## 🔧 PROCEDURES & CHECKS (optional)
            3–6 concise bullets focusing on steps, settings, checks, or cautions relevant to operating, setup, or replication. Omit if not applicable.

            ## ⚠️ LIMITATIONS & CAVEATS (optional)
            2–5 bullets on constraints, safety trade‑offs, or common pitfalls explicitly mentioned. Omit if none.

            ## 🗣️ QUOTABLE MOMENTS (optional)
            2–4 short quotes, ideally ≤ 16 words, with timestamps if inferable. Use double quotes. Omit if none.

            ## ✅ FACTS & TERMS TO KNOW
            List specific models, dates, definitions, or acronyms mentioned. If unclear in the transcript, write **Unknown** rather than guessing.

            ## 🔗 LINKS & REFERENCES (mentioned in video)
            Bullet any URLs or named resources explicitly referenced. If none, write **None mentioned**.

            **Formatting rules:**
            - Use British/Canadian spelling where applicable.
            - Strictly Markdown; no HTML; no additional headings.
            - Do not invent facts; if you are not sure, write **Unknown**.
            - Keep bullets tight; avoid line‑wrapped paragraphs in bullets.
            - Never repeat the same information across multiple sections.
            - Omit any optional section if not applicable; do NOT include empty headings.
            """,
