    ...
    <script>
        ...
        function initializeFilterSystem() {
            ...
            document.querySelectorAll('#modelsList input[type="checkbox"]').forEach(cb => {
                cb.addEventListener('change', () => updateFilterState('models', cb.value, cb.checked));
            });
            
            document.querySelectorAll('#channelsList input[type="checkbox"]').forEach(cb => {
                cb.addEventListener('change', () => updateFilterState('channels', cb.value, cb.checked));
            });
        ...
        function saveFilterStateToURL() {
            ...
            if (filterState.models.size > 0) {
                params.set('models', Array.from(filterState.models).join(','));
            }
            if (filterState.channels.size > 0) {
                params.set('channels', Array.from(filterState.channels).join(','));
            }
        ...
        function loadFilterStateFromURL() {
            ...
            if (params.has('models')) {
                ...
                models.forEach(model => filterState.models.add(model));
            ...
            if (params.has('channels')) {
                ...
                channels.forEach(channel => filterState.channels.add(channel));
