...
class ReportsHTTPHandler(SimpleHTTPRequestHandler):
    ...
    def send_index_page(self):
        ...
        try:
            ...
            # Calculate retention period for display
            retention_days = self.bot_instance.HTML_TTL // 86400
            retention_hours = (self.bot_instance.HTML_TTL % 86400) // 3600
            if retention_days > 0:
                retention_text = f"{retention_days} day{'s' if retention_days != 1 else ''}"
                if retention_hours > 0:
                    retention_text += f" and {retention_hours} hour{'s' if retention_hours != 1 else ''}"
            else:
                retention_text = f"{retention_hours} hour{'s' if retention_hours != 1 else ''}"
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        @media (max-width: 768px) {{
            ...
            .content, .stats, .footer {{ padding: 20px; }}
...
<body>
    <div class="container">
        ...
        <div class="stats">
            ...
            <div class="stat-card">
                <div class="stat-number">{retention_text}</div>
        ...
        <div class="content">
            ...
            if html_files:
                ...
                for file_path in html_files:
                    ...
                    # Try to extract title and URL from HTML file content
                    try:
                        ...
                        logger.warning(f"Could not extract title from {file_path.name}: {e}")
                    ...
                    index_html += f"""
                <div class="report-card" data-title="{display_name}" data-time="{mtime.timestamp()}" data-size="{file_size}" data-filename="{file_path.name}">
                    ...
                    <div class="report-meta">
                        ...
                        <div class="report-meta-item">
                            <span>💾</span> {file_size:.1f} KB
                        </div>
                        <div class="report-meta-item">
                            <span>📄</span> {file_path.name}
                        </div>"""
                    ...
                    index_html += f"""
                        <a href="/{file_path.name}" class="btn btn-secondary" target="_blank">🔗 Open in New Tab</a>
                    </div>
                </div>"""
                index_html += '</div>'
            else:
        ...
        <div class="footer">
            <p><strong>YouTube Summarizer Bot</strong> • Powered by AI • Reports are automatically cleaned up after """ + retention_text + """</p>
...
class YouTubeTelegramBot:
    ...
    def __init__(self, token: str, allowed_user_ids: List[int]):
        ...
        self.web_port = self._find_available_port(default_port)
...
<body>
    <div class="container">
        ...
        <div class="footer">
            ...
            <p>AI Model: {processor_info.get('llm_provider', 'Unknown')}/{processor_info.get('model', 'Unknown')}</p>
        </div>
    </div>
</body>
</html>"""
    ...
    def _cleanup_expired_reports(self):
        """Remove HTML reports older than HTML_TTL"""
        try:
            current_time = time.time()
            for file_path in self.exports_dir.glob("*.html"):
                if current_time - file_path.stat().st_mtime > self.HTML_TTL:
                    file_path.unlink()
        ...
        if callback_data == "new_request":
            ...
            try:
                ...
                if self.last_video_url:
                    ...
                    try:
                        # Create a safe callback for the last video with adaptive processing
                        adaptive_callback = self._create_safe_callback_data("process_adaptive", self.last_video_url)
                        
                        # Create a keyboard with just the auto-process option
                        keyboard = [[InlineKeyboardButton("🔄 Process with New Model", callback_data=adaptive_callback)]]
                        reply_markup = InlineKeyboardMarkup(keyboard)
                        
                        await query.edit_message_text(
                            f"✅ **Model Changed Successfully!**\n\n" +
                            f"**New Model:** {provider}/{model}\n\n" +
                            "🔄 Click below to re-process your last video with the new model:",
                            reply_markup=reply_markup,
                            parse_mode=ParseMode.MARKDOWN
                        )
