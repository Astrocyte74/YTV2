...
<body>
                ...
                <div class="controls-row">
                    ...
                    <div class="controls-group">
                        ...
                        <div class="filter-info" style="color: #6c757d; font-size: 0.9em; margin-left: 12px;">
                            Use drawer filters to refine results
                        </div>
        ...
        <div class="content">
            ...
            <div class="toolbar">
                <div class="toolbar-left">
                    ...
                    <div class="filter-info" style="color: #6c757d; font-size: 0.9em;">
                    ...
                    <details open>
                        <summary>
                            ...
                            <span class="group-actions">
                                ...
                                <button type="button" data-clear="models">Clear</button>
                            </span>
                        </summary>
                        <div class="group">
                    ...
                    <details open>
                        <summary>
                            ...
                            <span class="group-actions">
                                ...
                                <button type="button" data-clear="channels">Clear</button>
                            </span>
                        </summary>
                        <div class="group">
    ...
    <script>
        ...
        function initializeFilterSystem() {
            ...
            // Group actions (Select All / Clear)
            document.querySelectorAll('[data-select-all]').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    const group = e.target.getAttribute('data-select-all');
            ...
            document.querySelectorAll('[data-clear]').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    const group = e.target.getAttribute('data-clear');
                    clearGroup(group);
                });
            });
        ...
        function selectAllInGroup(group) {
            ...
            document.querySelectorAll(`${listId} input[type="checkbox"]`).forEach(cb => {
                cb.checked = true;
                updateFilterState(group, cb.value, true);
            });
            updateUI();
        }
        ...
        function clearGroup(group) {
            ...
            document.querySelectorAll(`${listId} input[type="checkbox"]`).forEach(cb => {
                cb.checked = false;
                updateFilterState(group, cb.value, false);
            });
            updateUI();
        }
        ...
        // URL State persistence with short slugs
        function saveFilterStateToURL() {
            try {
                ...
                // Build URL - only include filters that exclude items
                const url = new URL(window.location.origin + window.location.pathname);
                
                if (filterState.models.size > 0 && filterState.models.size < totalModels) {
                    url.searchParams.set('m', modelSlugs.join(','));
                }
                if (filterState.channels.size > 0 && filterState.channels.size < totalChannels) {
                    url.searchParams.set('c', channelSlugs.join(','));
                }
                
                const finalUrl = url.toString();
                
                // If URL too long, use hash fragment to avoid server issues
                if (finalUrl.length > 1800) {
                    const payload = btoa(JSON.stringify(state));
                    const hashUrl = `${window.location.origin}${window.location.pathname}#f=${payload}`;
        ...
        function loadFilterStateFromURL() {
            try {
                ...
                // 1) Try hash payload first
                if (window.location.hash.startsWith('#f=')) {
                    try {
                        const payload = window.location.hash.slice(3);
                        const decoded = JSON.parse(atob(payload));
                        
                        // Convert slugs back to original values
                        state.models = (decoded.models || [])
                ...
                // 2) Try query params
                if (!state.models && !state.channels) {
                    const params = new URLSearchParams(window.location.search);
                    
                    if (params.has('m')) {
                        const modelSlugs = params.get('m').split(',').filter(Boolean);
                        state.models = modelSlugs
                            .map(slug => slugMaps.modelSlugs.get(slug))
                            .filter(Boolean);
                    }
                    
                    if (params.has('c')) {
                        const channelSlugs = params.get('c').split(',').filter(Boolean);
                        state.channels = channelSlugs
                            .map(slug => slugMaps.channelSlugs.get(slug))
                            .filter(Boolean);
                    }
                }
                ...
                if (state.models && state.models.length > 0) {
                    ...
                    state.models.forEach(model => filterState.models.add(model));
                ...
                if (state.channels && state.channels.length > 0) {
                    ...
                    state.channels.forEach(channel => filterState.channels.add(channel));
