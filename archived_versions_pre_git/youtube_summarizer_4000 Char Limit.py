...
class YouTubeSummarizer:
    ...
    async def _condense_for_tts(self, text: str) -> str:
        ...
        condense_prompt = f"""
        Condense the following detailed summary into approximately 350 words while preserving:
        - The main findings and conclusions
        - Specific numbers, prices, and key facts
        - The final recommendation/bottom line
        - Natural conversational flow for audio
        
        Make it sound like a concise but comprehensive verbal summary, not a bullet-point list.
    ...
    async def generate_tts_audio(self, text: str, output_filename: str = None) -> Optional[str]:
        ...
        try:
            ...
            tts_text = text
            if len(text) > 4000:  # Leave buffer for safety
                print(f"📝 Audio summary is {len(text)} characters (exceeds 4096 limit)")
