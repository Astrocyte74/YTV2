    ...
    <script>
        ...
        const playlist = {
            ...
            discoverTracks() {
                ...
                this.tracks = Array.from(reportCards)
                    .filter(card => card.dataset.summaryType === 'Audio')
                    .map((card, index) => {
                        const filename = card.dataset.filename;
                        const audioUrl = filename ? `/exports/${filename.replace('.html', '.mp3')}` : null;
                        
                        return {
                            card: card,
                            title: card.dataset.title || `Audio Summary ${index + 1}`,
                ...
                tracksContainer.innerHTML = this.tracks.map((track, index) => `
                    <div class="playlist-track" data-index="${index}">
                        ...
                        <div class="track-controls">
                            <button class="track-play-btn" onclick="playlist.playTrack(${index})">
                                ▶ Play
                            </button>
                        </div>
                    </div>
